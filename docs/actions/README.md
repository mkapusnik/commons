...
| Lifecycle Tag Gate | `.github/actions/tag-gate` | Decide whether a lifecycle promotion should run by comparing source and target tags. | [tag-gate.md](tag-gate.md) |
...
- API-based actions use `actions/github-script@v8`; shell-based actions assume the runner has `git` available.
...
- Tag gate reads refs with `git ls-remote`; `contents: read` is enough for same-repository public or checked-out private usage, while private cross-repository remotes need credentials in the remote URL or checkout configuration.
